...
def pointwise_test(data, significance=0.05, standardize=True, plot=False):
    if standardize:
    ...
    nn = NearestNeighbors(k1, metric='cosine').fit(data)
    ...
    for i in candidates:
        mu = data[i, :]
        _, ind = nn.kneighbors(mu.reshape(1,-1))
        ind = ind.ravel()
        X = data[ind[:k2], :]
        if hotelling_pval(X, mu) < 0.05:
            non_modes.append(i)
    ...
    for i in range(K):
        ...
        for j in range(i+1, K):
            ...
            x = data[modes[i], :]
            y = data[modes[j], :]
            alpha = np.linspace(0, 1, 4000).reshape(-1, 1)
