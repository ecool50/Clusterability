---
title: "Data Analysis Postive Control"
...
obj <- .CreateSeuratObj("Data/SPF_allTcells.txt", Pname = "SPF_ALL", minCells = 5, minGenes = 200, matrix = F, sep = "\t")
...
new_varGenes <- fread("Data/spf_all_var_genes.txt", header = F, col.names = "Genes")

#get the data containing the top genes
obj_sub <- .GetGeneData(obj, varGenes$gene)
#obj_sub_new <- as.matrix(obj@scale.data[new_varGenes$Genes, ])
...
assignments <- cutree(as.hclust(dend), 2)
...
#get the number of PCs with variance greater than alpha = 0.05
#top_pcs <- sum(pca_inf["proportion",] >= 0.050)


#extract these PCs
#data_top_PCs <- data_pca <- as.matrix(GetCellEmbeddings(object = obj, reduction.type = "pca", dims.use = 1:top_pcs))

#run the diptest
#dip.test(parallelDist(data_top_PCs))
```
...
```{r}
pdf("Results/SPF_ALL_Density_Plots_new.pdf")
...
hist(varGenes_dist, main = paste("SPF All T-Cells density plot on all variable genes. \nDip Test p-value =", round(varGenes_dip, 4)), xlab = "Distance", probability = T, col = "lightblue")
...
```{r}
hist(pca_dist, main = "SPF ALL Density Plot", xlab = "Distance", probability = T, col = "red")
lines(density(pca_dist), col="blue", lwd=2) 
```
```{r}
hist(data_dist, main = "SPF ALL Density Plot top 350 genes", xlab = "Distance", probability = T, col = "red")
lines(density(data_dist), col="blue", lwd=2) 
```

```{r}
full_dist <- parallelDist(t(obj_sub))
hist(full_dist, main = "SPF ALL Density Plot all genes", xlab = "Distance", probability = T, col = "red")
lines(density(full_dist), col="blue", lwd=2) 
```

```{r}
#full_dist <- parallelDist(t(obj_sub))
hist(dist_list[[1]], main = "SPF ALL Density Plot all genes", xlab = "Distance", probability = T, col = "red")
lines(density(dist_list[[1]]), col="blue", lwd=2) 
```

# Lets try using a non linear dimension reduction using MDS
```{r}
#lets try classic
classic_mds <- t(obj_sub) %>% parallelDist() %>%         
  cmdscale(k = 1) %>%
  as.matrix()

#classic_mds_dist <- parallelDist(as.matrix(classic_mds))

# Kruskal’s non-metric multidimensional scaling
kruskal_mds  <- t(obj_sub) %>%
  dist() %>%          
  isoMDS(k = 1) %>%
  .$points %>%
  as.matrix()
...
hist(pc_1, main = "SPF ALL T-Cells Density Plot of the first PC", xlab = "Value", probability = T, col = "red")
...
# Save the plots
```{r}
pdf("SPF_ALL_T_Cells.pdf")

plot(result$plot, use_labs = F) + ggtitle("Significance clustering on top 350 variable genes")
plot(dend, type = "rectangle", ylab = "Height", edgePar = list(col = 2:3, lwd = 2:1),main = "Consensus Tree, 4.6e-04",  leaflab = "none")
...
hist(full_dist, main = "Density Plot on top 350 variable genes", xlab = "Distance", probability = T, col = "red")
lines(density(full_dist), col="blue", lwd=2)
...
# Compute 1d cluster and plot it on tsne
```{r}
result <- Ckmeans.1d.dp(data_tsne[,2], 2)
...
write.csv(all_labels, file = "SPF_ALL_Tcells_Clustering_labels.csv")
```

```{r}
pdf("Possible_Density_plot_themes.pdf")
p
p1
p2
p4
...
# plot the histogram of the pairwise distances of the significant PCs
```{r}
sig_pcs_dat <- as.matrix(GetCellEmbeddings(object = obj, reduction.type = "pca", dims.use = 1:sig_pcs))
dist_sig_pcs <- melt(as.matrix(parallelDist(sig_pcs_dat)))
p2 <- ggplot(dist_sig_pcs, aes(x=value)) + 
 geom_histogram(aes(y=..density..), fill="lightblue", binwidth=1, colour = "black")+
  geom_line(stat="density", colour = "red") + 
  theme_classic() +  
  labs(title="SPF ALL Density Plot on Top Significant PCs. Dip Test p-value < 2.2e-16",x="Distance", y = "Density") +
...
hist(fit$lambda, breaks = 200, main = paste("SPF All T-Cells density plot on Principal Curves. \nDip Test p-value =", round(fit_dip, 4)), xlab = "Distance", probability = T, col = "lightblue")
...
pdf("Results/SPF_ALL_Tcells_All_Density_Z_score_plots.pdf")
for(i in seq_vals){
  #first run PCA
...
hist(pca_dist, breaks = 300, main = paste("SPF All T-Cells  density plot on significant PCs on", i, "variable genes", "\nDip Test p-value =", round(pca_dip,4)), xlab = "Distance", probability = T, col = "lightblue")
lines(density(pca_dist), col="red", lwd=2)
...
X <- t(obj_sub)
sol <- mddr(X, 10)
# pdf("Results/SPF_ALL_Tcells_All_Hyperplane_plots_Sig_PCs.pdf")
plot(sol)
sol_dist <- parallelDist(sol$fitted)
dip_stat <- dip.test(sol_dist)$p.value

sol_corr <- (1 - WGCNA::cor(t(sol$fitted)))
corr_dip <- dip.test(sol_corr)$p.value

plot(sol$fitted[, 1], sol$fitted[, 2], xlab = "Plane 1", ylab = "Plane 2", main = "Minimum density hyperplane data projection")
hist(sol_dist, breaks = 100, main = paste("SPF ALL Tcells All Sig PCs density plot on Min Density Hyperplane", "\nDip Test p-value =", round(dip_stat,4)), xlab = "Distance", probability = T, col = "lightblue")
lines(density(sol_dist), col="red", lwd=2)

plot(density(sol_corr), main = paste("Dip pval = ", round(corr_dip, 4)))
#dev.off()
```
...
pdf("Results/SPF_noCC_Min_Density_hyperplane_Sig_PCs_plots.pdf")
for(i in seq_vals){
...
hist(sol_dist, breaks = 300, main = paste("SPF No CC density plot on significant PCs on", i, "variable genes", "\nDip Test p-value =", round(dip_stat,4)), xlab = "Distance", probability = T, col = "lightblue")
lines(density(sol_dist), col="red", lwd=2)
...
for(i in gene_seq){
  ...
  sim <- splatSimulate(params, nGenes = i)
...
result_table <- data.frame(gene_seq[1:90], full_res_dip_stat, full_res, full_res_eucl , full_res_eucl_dip_stat, mdh_res_eucl, mdh_res_man, mdh_res_p, pc_res)
...
# run SCRNA
 res <- .RunScrna(counts_data = counts, dim = 2, nGenes = 1000)
  ...
  # run the dip test on the results
  plot(res$pca_residuals$factors[, 1:2], main = "SCRNA PCA Plot")
  glm_dist <- parallelDist(as.matrix(res$pca_residuals$factors[,1:2]))
  glm_dip <- dip.test(glm_dist)$p.value
  plot(density(glm_dist), xlab = "Distance", ylab = "Density", main = paste("GLM-PCA Dip pval = ", round(glm_dip, 4)))

  glm_corr <- (1 - WGCNA::cor(t(res$pca_residuals$factors)))
  glm_corr_dip <- dip.test(glm_corr)$p.value
  
  # run MDH on the residuals  
  glm_mdh <- mdh(t(res$residuals), v0 = res$pca_residuals$loadings[, 1])
  plot(glm_mdh)
  glm_mdh_dist <- parallelDist(glm_mdh$fitted)
  glm_mdh_dip <- dip.test(glm_mdh_dist)$p.value
  plot(density(glm_mdh_dist), xlab = "Distance", ylab = "Density", main = paste("Residual-MDH Dip pval = ", round(glm_mdh_dip, 4)))
