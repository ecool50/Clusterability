...
obj <- .CreateSeuratObj("Data/SPF.txt", Pname = "NIP", minCells = 5, minGenes = 200, matrix = F, sep = "\t")
...
seq_vals <- seq(50,100, by = 50)

#iterate over sequence values and run program
results <- NULL
for(val in seq_vals){
  result <- .run_dip(val, obj_sub, varGenes)
...
output <- .Boot_Con(temp, obj, distance="euclidean", method="ward",nboot=100)
...
assignments <- cutree(output$dendrogram, 2)

#run the significance testing 
Sig_Res <- sigclust(temp, labels = assignments)
...
# Lets compute HVG another way

```{r}
counts <- obj@raw.data

#use scVEGS to compute highly variable genes
hvgs <- scVEGs(counts, pVal = 0.05, pFlag = 1, species = "blah",tmpName = "SPF_scVEGS")
```
