...
encode = function(dat, seed = 1994, max_random_projection = 2048, encoded_dim = 16, hidden_dims = c(256), learning_rate = 0.001, batch_size = 256, epochs = 50, verbose = 2, scale = FALSE, genes_as_rows = FALSE) {
  if (verbose[1] %in% 0:2) {
    verbose = verbose[1]
  } else {
    verbose = 1
  ...
  for (layer_width in hidden_dims) {
    ...
    tns = keras::layer_activation_elu(tns, alpha = 0.01)
  }
