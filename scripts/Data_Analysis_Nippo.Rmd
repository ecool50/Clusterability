...
obj <- .CreateSeuratObj("Data/Nippo.txt", Pname = "Nippo", minCells = 5, minGenes = 200, matrix = F, sep = "\t")
...
# Run the first test
```{r}
#result <- .run_dip(nrow(obj_sub), obj_sub, varGenes = varGenes)
...
SPF
#plot it
plot(dend, type = "rectangle", ylab = "Height", edgePar = list(col = 2:3, lwd = 2:1), leaflab = "none", main = "Nippo Consensus Tree")
...
```{r}
pdf("Results/Nippo_Density_Plots.pdf")
...
hist(varGenes_dist, main = paste("Nippo density plot on all variable genes. Dip Test p-value =", round(varGenes_dip, 4)), xlab = "Distance", probability = T, col = "lightblue")
...
hist(pca_dist, main = paste("Nippo density plot on significant PCs. Dip Test p-value =", round(pca_dip,4)), xlab = "Distance", probability = T, col = "lightblue")
...
hist(pc_1, main = paste("Nippo density plot on the first PC. Dip Test p-value =", round(pc_1_dip,4)), xlab = "Distance", probability = T, col = "lightblue")
...
pca_dist <- parallelDist(data_pca) 
dip_pca <- dip.test(pca_dist)$p.value
```

```{r}
dist_full <- parallelDist(t(obj_sub[varGenes$gene[1:350], ]))
hist(dist_full, main = "Nippo Density Plot", xlab = "Distance", probability = T, col = "red")
...
hist(pc_1, main = "Nippo Density Plot of the first PC", xlab = "Value", probability = T, col = "red")
